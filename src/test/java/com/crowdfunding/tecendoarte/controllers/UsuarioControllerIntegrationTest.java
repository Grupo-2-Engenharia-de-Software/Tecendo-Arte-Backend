...
class UsuarioControllerIntegrationTest {
        ...
        @Test
        void atualizarUsuario_comIdContaInvalido_deveRetornar400() throws Exception {
                // Cria um usuário válido
                Long contaId = criarConta();
                Usuario usuario = usuarioRepository.findByConta(contaRepository.findById(contaId).get()).get();
                Long usuarioId = usuario.getId();

                // Tenta atualizar o usuário com um DTO contendo um ID de conta inexistente
                UsuarioRequestDTO req = new UsuarioRequestDTO();
                req.setContaId(99999L);
                req.setInteresses(List.of(TipoArte.PINTURA));

                String response = mockMvc.perform(put("/usuarios/{id}", usuarioId)
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(objectMapper.writeValueAsString(req)))
                                .andExpect(status().isBadRequest())
                                .andReturn()
                ...
                assertThat(response).contains("ID da conta");
        }
}